...
export function classifyQuestion(question) {
  ...
  const isCorpusWideSynthesis =
    /\b(across|throughout)\b.*\b(guides|publications|documents|sources|corpus)\b/.test(
      q
    ) ||
    /\b(recur|recurs|recurring|shared|common|cross cutting|synthesi[sz])\b.*\b(guides|publications|documents|sources|corpus)\b/.test(
      q
    ) ||
    /\b(themes|patterns|principles)\b.*\b(across|throughout|shared|common|recur)\b/.test(
      q
    );
  ...
  return { isListQuery, isCrossDocQuery, isCorpusWideSynthesis, isNameQuery };
}
...
function corpusWideSeeds(scored, sourceInfos, controls, isCorpusWideSynthesis) {
  if (!isCorpusWideSynthesis) return [];

  let relevantSources = sourceInfos;
  if (controls.mentions.guides) {
    relevantSources = relevantSources.filter((info) => info.isGuide);
  } else if (controls.mentions.csp && !controls.mentions.paper) {
    relevantSources = relevantSources.filter((info) => info.isCsp);
  }

  return relevantSources
    .map((info) => scored.find((hit) => hit.source === info.source))
    .filter(Boolean);
}
...
export function searchCorpus(
  ...
  const { isCrossDocQuery, isCorpusWideSynthesis } =
  ...
  const broadSeeds = corpusWideSeeds(
    scored,
    sourceInfos,
    controls,
    isCorpusWideSynthesis
  );
  const hits = selectFinalHits(candidatePool, finalK, [...seeds, ...broadSeeds]);
...
export function getCuratedResponse(question, manifest, curatedContext) {
  ...
  const asksCspInventory =
    /\b(?:list|enumerate|name|show)\b.*\bcsp\b.*\b(?:guides|publications|documents|sources)\b/.test(
      q
    ) ||
    /\b(?:what|which)\s+csp\s+(?:guides|publications|documents|sources)\b.*\b(?:corpus|index|indexed|include|included|available|access|retrieve|trained|have)\b/.test(
      q
    ) ||
    /\bcsp\s+(?:corpus|inventory)\b/.test(q);
  if (asksCspInventory) {
